...
int main(int argc, const char *argv[]) {//argc,argv[]: https://blog.csdn.net/dcrmg/article/details/51987413
  if (argc < 2) {
  ...
  //    varible_type    variable_name         ("param_name","param_default_value")
  ...
  std::string action(paramAction(config));//action is a varible name
  //can we also write like:
...
/*****************************************************************************/
    ...
    if (action == "train-nn" or action == "visualize-nn") {
      ...
      //Sheet03 Task05 Prior Computation
      ...
      for (size_t batch = 0ul; batch < mini_batch_builder.num_train_batches(); batch++) {
          ...
          for (size_t i = 0; i < nn.get_batch_mask().size(); i++)
            for (size_t j = 0; j < nn.get_batch_mask()[i]; j++) {
                ...
                //slice(a,b,c): from a select b elements with step c
          }
      }
