...
MiniBatchBuilder::MiniBatchBuilder(Configuration const& config, Corpus const& corpus, size_t batch_size, size_t num_classes, StateIdx silence_state)
  ...
  std::iota(train_segment_sequence_.begin(), train_segment_sequence_.end(), 0ul);//to generate a continuous value
  shuffle();
...
const ParameterFloat AdaDeltaUpdater::paramLearningRate("learning-rate", 0.001);//todo::default:1

void AdaDeltaUpdater::update() {
    //Sheet3 Task3
    float eps = 0.000001;
      for(const auto& lnameParam : parameters_) {//the out loop is iterate every layer
